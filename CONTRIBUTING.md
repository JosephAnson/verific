...
You can fork this [StackBlitz template](https://stackblitz.com/edit/verific-issue-template?file=src%App.vue) to get a starting working environment ready for your demo with all verific packages pre-installed and using the latest release.
...
If you are working on a specific package within the verific mono repo and only want to build that, then use the following command to build specific packages, the package id is the folder name in the `packages` folder.
...
- Build verific dist files once you are done with your changes
